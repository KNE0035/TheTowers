...
public  abstract class DefenseTowerWithProjectile extends DefenseTower{
    ...
    DefenseTowerWithProjectile(PointF position, PointF imageOffset, float direction, int radius, int price, List<ModelObject> activeObjects, int rateOfFireMillis, int windowGapMillis, ProjectileType projectileType, List<Projectile> projectiles, boolean enemy, final int damage, final ArmorType armorCounter) {
    ...
    private Projectile resolveProctileTypeAndCreateProjectile(ProjectileType type){
        ...
        if(null != type) switch (type) {
            case Cannon_Ball:
                CannonBall ball = new CannonBall(projectileStartPosition, new PointF(100, 0), (Unit)this.lockedEnemy, this.damage);
                ball.setDirectionToPoint(this.lockedEnemy.getPosition());
                return ball;
            case Missile:
                Missile missile = new Missile(projectileStartPosition, new PointF(100, 0), (Unit)this.lockedEnemy, this.damage);
                missile.setDirectionToPoint(this.lockedEnemy.getPosition());
                return missile;
            case Photon_Ball:
                projectileStartPosition = this.getPosition();
                PhotonBall photonBall = new PhotonBall(projectileStartPosition, new PointF(100, 0), (Unit)this.lockedEnemy, this.damage);
